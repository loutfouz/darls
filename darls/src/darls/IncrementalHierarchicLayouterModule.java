 ...
 */
public class IncrementalHierarchicLayouterModule {
  ...
  public IncrementalHierarchicLayouterModule(Graph2DView view) {
	this.view = view;  
    ...
    /* disabled because benedikt didnt like that they were inconsistent with optimal condition in dual view.
     * lets keep it consistent by disabling them.
    ...
    hierarchicLayouter.setOrthogonallyRouted(false);
...
/*
  private void initGraph(Graph2D graph) {
    ...
    Node n2 = graph.createNode(100, 0);
  ...
  public void optimizeNodes(NodeCursor nc)
  {
	  OptimizeNodesAction ona = new OptimizeNodesAction("Optimize Node", nc, true);
	  ona.actionPerformed(null);
  }
  public void fixNodes(NodeCursor nc)
  {
	  //first layer then sequence
	  FixNodesAction fna = new FixNodesAction("Fix Coordinates", nc, true, true);
	  fna.actionPerformed(null);
  ...
  protected void setViewModes() {
    editMode = new IncrementalEditMode();
  ...
  final class FixNodesAction extends AbstractAction {
    ...
    //November 18, 2010 - I uncommented all the node coloring stuff
    public void actionPerformed(ActionEvent ae) {
      this.nc.toFirst();
      for (NodeCursor nc = this.nc; nc.ok(); nc.next()) {
        Node v = nc.node();
  ...
  static final class LayerDrawable implements Drawable {
    ...
    private static final Color[] colors = new Color[]{new Color(150,200,255,128), new Color(220,240,240,128)};
    ...
    public Rectangle2D getLayerBounds(int layer) {
      ...
      return new Rectangle2D.Double(bounds.x - 2 * outerInsets, bounds.y - 2 * outerInsets,
          bounds.width + 4.0d * outerInsets, bounds.height + outerInsets * 4.0d);
    }
...
public void relayoutCreatedNode(Node v) {
      ...
      //formerly (because we add a new node at the bottom:
      //setLayers(new NodeList(v).nodes(), Integer.MAX_VALUE, Integer.MAX_VALUE);
...
public void relayoutCreatedEdge(Edge edge) {
  ...
  /*public static void main(String[] args) {
    EventQueue.invokeLater(new Runnable() {
      public void run() {
        initLnF();
        (new IncrementalHierarchicLayouterModule()).start("Incremental Hierarchic Layouter Demo");
      }
    });
  }*/
}
